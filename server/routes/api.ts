...
router.post('/courses', async (req: AuthRequest, res) => {
  try {
    const id = uuidv4();
    const { name, entity, modality, location, startDate, endDate, totalHours, pricingType, price, status, color } = req.body;
    await pool.query(
      'INSERT INTO courses (id, user_id, name, entity, modality, location, start_date, end_date, total_hours, pricing_type, price, status, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, req.user?.id, name, entity, modality, location, startDate, endDate, totalHours, pricingType, price, status, color]
    );
...
router.put('/courses/:id', async (req: AuthRequest, res) => {
  try {
    const { name, entity, modality, location, startDate, endDate, totalHours, pricingType, price, status, color } = req.body;
    await pool.query(
      'UPDATE courses SET name=?, entity=?, modality=?, location=?, start_date=?, end_date=?, total_hours=?, pricing_type=?, price=?, status=?, color=? WHERE id=? AND user_id=?',
      [name, entity, modality, location, startDate, endDate, totalHours, pricingType, price, status, color, req.params.id, req.user?.id]
    );
...
router.post('/sessions', async (req: AuthRequest, res) => {
  try {
    const id = uuidv4();
    const { courseId, date, startTime, endTime, content } = req.body;
    await pool.query(
      'INSERT INTO sessions (id, course_id, user_id, date, start_time, end_time, content) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, courseId, req.user?.id, date, startTime, endTime, content]
    );
    res.json({ id, ...req.body, userId: req.user?.id });
...
router.get('/settings', async (req: AuthRequest, res) => {
  try {
    ...
    if (rows.length === 0) {
      res.json({ maxHoursPerWeek: 40, minHourlyRate: 0, availableDays: [], preferredModality: 'any' });
    } else {
...
router.put('/settings', async (req: AuthRequest, res) => {
  try {
    const { maxHoursPerWeek, minHourlyRate, availableDays, preferredModality } = req.body;
    const [existing] = await pool.query<any[]>('SELECT user_id FROM settings WHERE user_id = ?', [req.user?.id]);
    
    if (existing.length === 0) {
      await pool.query(
        'INSERT INTO settings (user_id, max_hours_per_week, min_hourly_rate, available_days, preferred_modality) VALUES (?, ?, ?, ?, ?)',
        [req.user?.id, maxHoursPerWeek, minHourlyRate, JSON.stringify(availableDays), preferredModality]
      );
    } else {
      await pool.query(
        'UPDATE settings SET max_hours_per_week=?, min_hourly_rate=?, available_days=?, preferred_modality=? WHERE user_id=?',
        [maxHoursPerWeek, minHourlyRate, JSON.stringify(availableDays), preferredModality, req.user?.id]
      );
    }
    res.json({ success: true });
